...
func (o bootstrapOptions) resolveRepos() ([]string, error) {
	...
	for _, spec := range o.repos {
		...
		if !filepath.IsAbs(path) {
			if _, err := os.Stat(path); err != nil { // not a path relative to CWD — try the projects dir
				path = filepath.Join(o.projectsDir, spec)
			}
		}
...
// contract).
func composePoolArgs(bootOpts bootstrapOptions) []string {
	args := []string{
		"agent-pool",
		"--projects-dir", bootOpts.projectsDir,
		"--repos", strings.Join(bootOpts.repos, ","),
		"--concurrency", strconv.Itoa(bootOpts.agents),
		"--max-concurrent-agents", strconv.Itoa(bootOpts.agents),
		"--interval", bootOpts.interval.String(),
		"--daily-budget", strconv.Itoa(bootOpts.dailyBudget),
		"--max-per-tick", strconv.Itoa(bootOpts.maxPerTick),
		"--yolo=" + strconv.FormatBool(bootOpts.yolo),
		"--review=" + strconv.FormatBool(bootOpts.review),
		"--review-autofix=" + strconv.FormatBool(bootOpts.reviewAutofix),
		"--project-exclusive=" + strconv.FormatBool(bootOpts.exclusive),
		"--allow-dirty=" + strconv.FormatBool(bootOpts.allowDirty),
	}

	if bootOpts.repoTimeout != "" {
		args = append(args, "--repo-timeout", bootOpts.repoTimeout)
	}

	if bootOpts.repoInterval != "" {
		args = append(args, "--repo-interval", bootOpts.repoInterval)
	}

	if bootOpts.dlqBackoff > 0 {
		args = append(args, "--dlq-backoff", bootOpts.dlqBackoff.String())
	}

	// Deliberately NO "--model" here even when bootOpts.model is set: the managed
	...
	if bootOpts.logDir != "" {
		args = append(args, "--log-dir", bootOpts.logDir)
	}

	if bootOpts.logDirMaxAge > 0 {
		args = append(args, "--log-dir-max-age", bootOpts.logDirMaxAge.String())
	}

	if bootOpts.logDirMaxBytes > 0 {
		args = append(args, "--log-dir-max-bytes", strconv.FormatInt(bootOpts.logDirMaxBytes, 10))
	}

	if bootOpts.db != "" {
		args = append(args, "--db", bootOpts.db)
	}
...
func renderPoolConfig(bootOpts bootstrapOptions) string {
	var b strings.Builder
