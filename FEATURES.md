...
| Feature                        | Status                | Notes                                                                        |
| ------------------------------ | --------------------- | ---------------------------------------------------------------------------- |
| Nix flake build + vendor-hash check | 🟡 `PARTIALLY_FUNCTIONAL` | `flake.nix` pins `CGO_ENABLED=0` and a vendor hash with a fast drift gate; nix gates have not been re-run since the latest go.mod/go.sum changes |
