...
| Agent reviews (`--review`, `--review-autofix`)                                       | 🟢 `FULLY_FUNCTIONAL`     | `internal/executor/review.go` + `internal/review`: every completed agent task gets ONE review by a second agent; strict `TQ_RESULT` verdict contract (approve / request_changes + findings) lands in the completion fact. Autofix mints deduped fix tasks per finding. Reviews are never reviewed (structural loop guard) and every enqueue counts against the budget. Sweeper starts at journal head — completions before pool start are not replayed |
...
| Release runner                      | 🟢 `FULLY_FUNCTIONAL` | `scripts/release.sh vX.Y.Z [--tag|--push]` codifies the v0.1.0 checklist: full ci-local gates + go.mod hygiene + CHANGELOG-section checks, annotated tag from the CHANGELOG, proxy + clean-room `go get` verification, pre-release GitHub Release; safe default runs gates only |
