...
// tq-side companion surfaces over the SAME database (RecordAnswer/questions,
// the priority_scores cache, the CountFacts/FactsSince/LastFacts/ProjectCounts
// read pushdowns, AppendFact, watermark list/set, and tq's
// project-exclusivity claim).
//
...
//   - Adapter-side (same database, own pgx handle): ClaimDue
//     (upstream has no project exclusivity), Requeue (tq's evidence
//     carries resume_closeout), all reads, RecordAnswer, AppendFact,
//     watermarks, priority scores.
//
...
// adapter-implemented, not divergences); postgres-specific deltas land in
// the spike's own report.
package postgresv4
...
func Open(ctx context.Context, dsn string, opts ...StoreOption) (*Store, error) {
	...
	engine, err := upostgres.Open[[]byte](ctx, dsn, 1, upostgres.WithCodec(identityCodec()))
	if err != nil {
...
func (s *Store) migrateCompanion(ctx context.Context) error {
	if _, err := s.exec(ctx, companionSchema); err != nil {
...
func (s *Store) CountTasks(ctx context.Context, f queue.Filter) (int, error) {
	...
	err := s.queryRow(ctx, `SELECT COUNT(*) FROM tasks WHERE `+where, args...).Scan(&n)
...
func (s *Store) HeadSeq(ctx context.Context) (int64, error) {
	...
	err := s.queryRow(ctx, `SELECT COALESCE(MAX(seq), 0) FROM facts`).Scan(&seq)
...
func (s *Store) StatusCounts(ctx context.Context) (map[task.Status]int, error) {
	rows, err := s.query(ctx, `SELECT status, COUNT(*) FROM tasks GROUP BY status`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// pgq rewrites the sqlite-style "?" placeholders the shared adapter SQL is
// written in into postgres "$n" ordinals. String literals are respected
// (the adapter SQL contains none with "?", but the guard is cheap).
...
func scanTaskRow(r scanner) (task.Task, error) {
	...
	if deps != "" && deps != "[]" {
		if err := json.Unmarshal([]byte(deps), &t.Deps); err != nil {
			return task.Task{}, fmt.Errorf("postgresv4: unmarshal deps for %s: %w", id, err)
		}
	}
...
func leaseErr(ctx context.Context, q interface {
	...
	err := q.QueryRowContext(ctx, pgq(`SELECT status FROM tasks WHERE id = ?`), id.String()).Scan(&st)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return task.ErrNotFound
		}
