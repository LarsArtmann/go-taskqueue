...
func seedOldJournal(t *testing.T, path string) {
	...
	if err := store.Complete(ctx, claimed.ID, "worker-1", jsontext.Value(`{"ok":true}`)); err != nil {
	...
	deadClaimed, err := store.ClaimDue(ctx, "worker-2", time.Minute)
	if err != nil {
	...
	if err := store.Heartbeat(ctx, dead.ID, "worker-2", time.Minute); err == nil {
...
func TestVerifyMatchesSQLiteV4Store(t *testing.T) {
	...
	claimed, err := store.ClaimDue(ctx, "worker-after-cutover", time.Minute)
	if err != nil {
