...
if cmp -s "$TMP/main_head" "$TMP/wt_head"; then echo "FAIL: worktree commit leaked into main HEAD"; exit 1; fi
[ -f "$TMP/repo/wip.txt" ] || { echo "FAIL: main checkout mutated"; exit 1; }
[ ! -f "$TMP/repo/agent.txt" ] || { echo "FAIL: agent file landed in main checkout"; exit 1; }
