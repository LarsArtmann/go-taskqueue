...
- [ ] session-close bridge design + prototype: every INTERACTIVE crush session gets the same close-out as pool agents — `tq session begin/close` minting a journal fact + footer-based commit attribution (Crush-Session: <id>), a direct-enqueued review task over the session's commit range, and a done-prompt status task; trigger: crush PR charmbracelet/crush#3146 (SessionStart/SessionEnd hooks, OPEN/unmerged as of 2026-09-10; payloads carry session_id+cwd; SessionEnd has a 2s timeout so a merged-trigger hook must ONLY enqueue) — UNTIL it merges, the interim trigger is the PreToolUse session registry: CRUSH_SESSION_ID+CRUSH_CWD already arrive on every tool call, so a hook appends {id,cwd,last_seen} and a sweeper mints the close-out when no crush process owns the ID (tested 2026-09-10: undocumented SessionEnd/Stop hook names do NOT fire in the Claude-Code compat layer); attribution gap (daemon-folded commits) remains ours either way; the trust asymmetry (interactive = human-reviewed) predates interactive-sessions-also-being-Flash (2026-09-10 owner insight)
