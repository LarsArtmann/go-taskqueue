...
- [ ] CI reliability bundle: nix build + `nix flake check` job, TODO_LIST harvest-parse guard (`tq harvest --repos . --dry-run` must succeed), ghost-reference check that every path cited in AGENTS.md/README/FEATURES exists (plan C05)
