...
- [x] Verify that advisory lint findings actually surface as CI annotations on green runs (`continue-on-error` + annotation caps may swallow them); if not, adopt reviewdog or a scoped-lint step (19:33 report b/f6) — done: verified they do NOT surface (golangci-lint v2.13.2 emits zero annotation commands even with `GITHUB_ACTIONS=true`; the ~400-finding baseline would exceed GitHub's 10-warnings-per-step cap anyway), so adopted the scoped-lint option: `scripts/lint-annotations.sh` re-runs the same binary/config with `--new-from-rev` and emits findings on changed lines as `::warning` annotations, wired into ci.yml + ci-local.sh (reviewdog rejected: needs `checks:write`/`pull-requests:write` permission elevation and still caps unscoped)
