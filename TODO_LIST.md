...
- [ ] session-close bridge design + prototype: every INTERACTIVE crush session gets the same close-out as pool agents — `tq session begin/close` minting a journal fact + footer-based commit attribution (Crush-Session: <id>), a direct-enqueued review task over the session's commit range, and a done-prompt status task; investigate whether crush exposes a session-end hook event as the trigger (hooks system exists, event list unverified); the trust asymmetry (interactive = human-reviewed) predates interactive-sessions-also-being-Flash (2026-09-10 owner insight)
