...
| **Ask**                     | An agent's structured escalation to the owner mid-task (`tq ask --task <id>`): the question is redacted, journaled as `task.question-asked`, and written to the per-run `$TQ_QUESTION_FILE` marker. The question is a FACT, not a task.                                                                                                                        |
| **Question park**           | The owner-question sibling of the rate-limit park: a `QuestionPendingError` requeues WITHOUT burning an attempt, `not_before` = the question's expiry (`--expires`, default 72h, cap 7d) — expired questions re-enter the task as the safety valve when no answer comes.                                                                                       |
| **Qref**                    | The question's correlation key: sha256(task ID + normalized question text), truncated to 16 hex. Re-asking converges on the same ref; the answer routes home by ref alone, so the tokens (`task:` / `qref:`) LEAD the forwarded body where truncation can't sever them.                                                                                        |
| **Answer**                  | The owner's ruling arriving home: the bridge forwards the question to PapDashboard, the AnswerPoller polls answers back (at-least-once; `RecordAnswer` is idempotent per qref), the store injects it into the payload's `answered` array, clears `not_before`, and journals `task.question-answered` — the resumed run reads the ruling in its prompt.           |
...
| **Batch**         | One agent task covering up to N (`--batch-items`) _adjacent_ open items of the same TODO_LIST section, worked in order by ONE session. Dedup-keyed by the sorted member set (`batch:` prefix); a batch counts as one task against every gate. |
| **Tick**          | One pass of the pool loop: harvest, ingest bridges, then let workers claim. Budgets and caps bound each tick.                                                                                                                                 |
| **Work item**      | The harvested TODO_LIST item an agent task is minted for, carried in the agent payload (`item` + its dedup-key derivation). The unit a review quotes, a batch member contributes, and prune-stale checks for; absent only for non-harvested (operator/session) tasks.                                                                                                                                                            |
...
| **Retry trail**         | The chronological strip of a task's failure history — `task.requeued` (with `RequeueEvidence`) and `task.dead-lettered` facts in seq order, rendered by `tq show`/the detail page. Dead-lettered reasons COUNT into it; a task that died without retries still shows one strip entry.                                                                                       |
| **Payload section**     | The task detail page's rendering of the `Payload` JSON. Agent payloads WITH a work item lead with the item and keep the prompt COLLAPSED; item-less payloads lead WITH the prompt (no separate hint). Owner-approved 2026-09-11.                                                                           |
...
| **Derived outcome**      | What the queue learns about a finished run WITHOUT the agent reporting: commits via the `Task-Queue-ID` git footer, files via `git diff-tree` over them, session usage (cost/tokens/messages) via go-crush-data. Git is the source of truth for what SHIPPED; a self-report no longer exists (2026-09-14). |
| **Verdict channel**      | The agent→queue result channel for judgment tasks (review/status/dlqfix/prioritize): `tq verdict '<json>'` writes the run's `$TQ_RESULT_FILE`, which the executor appends as the LAST result line — it outranks any stdout line (the legacy fallback).                                                     |
