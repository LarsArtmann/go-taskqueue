...
> **EXECUTED — ARCHIVED 2026-09-21 (docs-health archive sweep)** — M4-M11 shipped (check-features-ci, cqrs-lint job, journalaudit, smokes); M-rows self-annotated DONE; residue routed rows 34/174/208.
...
| M4  | ~~**FEATURES CI-freshness gate**~~ **DONE 2026-09-13**: `scripts/check-features-ci.sh` (gh-verifies every `run NNNNNN` cited in FEATURES.md is completed/success), wired into ci-local + ci.yml, red-probed (bogus id → STALE exit 1), TODO row closed                                                                                                                                                                                                                                                                                                                                                          | 🔥🔥   | M      | Ops truth                   | 08-49 e1                                                            | 4%     |
| M5  | ~~**cqrs-lint advisory gate**~~ **DONE 2026-09-13**: ci-local advisory step (local-checkout provenance + flip criteria in comment) + ci.yml `cqrs-lint` advisory job (two-repo checkout, continue-on-error); clean run verified exit 0; TODO row closed                                                                                                                                                                                                                                                                                                                                                         | 🔥     | S      | Quality                     | minted row (08-49); 08-00 §c1                                       | 4%     |
| M6  | ~~**Docs-health batch 2**~~ **DONE 2026-09-13**: eligibility checked per report (all six windows' shipped work verified against the v0.3.0 CHANGELOG; §f/§g pointers live in TODO_LIST), inline annotations added, git-mv'd to archived/ (counter 34→40), index rows repointed, all 5 doc gates green                                                                                                                                                                                                                                                                                                           | 🔥     | M      | Historian                   | L165; 08-49 b2                                                      | 4%     |
...
| M8  | ~~**Journal-drift audit spike**~~ **DONE 2026-09-13**: `tq audit --journal` (cmd/tq/journalaudit.go) — fact-replay projection over the state-carrying facts, per-task status diff vs the store, text/JSON advisory output (exit 0 on drift by contract); two tests over a real sqlite store + live scratch-DB smoke; root vet + race green. 4th lint-baseline regen same day (886→888, +2 net: this window's journalaudit files entered clean; the +errcheck/+nlreturn residue is concurrent windows' committed files — facadeparity/examples/bridges — left as-is rather than touching others' in-flight code) | 🔥🔥   | L      | Correctness                 | minted row (08-49); 08-01 §f5                                       | 20%    |
...
_Rows that are pure owner decisions are routed to M15's package rather than worked — an agent cannot rule itself._
