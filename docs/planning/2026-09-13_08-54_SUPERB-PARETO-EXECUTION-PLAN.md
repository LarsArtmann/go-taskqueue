...
| M8 | ~~**Journal-drift audit spike**~~ **DONE 2026-09-13**: `tq audit --journal` (cmd/tq/journalaudit.go) — fact-replay projection over the state-carrying facts, per-task status diff vs the store, text/JSON advisory output (exit 0 on drift by contract); two tests over a real sqlite store + live scratch-DB smoke; root vet + race green | 🔥🔥 | L | Correctness | minted row (08-49); 08-01 §f5 | 20% |
