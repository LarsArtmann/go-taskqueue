...
- **Source documents**: each task's close-out report (read first, never re-derived), verified against HEAD by spot-checks: all six footer commits exist (`a1b4588`, `98417c0`, `88abe273`, `896339b`, `02f6c14`, `0fd2cf2`); CHANGELOG entries for the pin, the redaction pass (+depbump delegation, +span-merge semantics), and the ratelimit-e2e second-claim pin are present; TODO rows 88/90/91 are `[x]` with evidence.
...
1. **The daemon-fold attribution wart is now FOUR instances** (a1b4588, 896339b, c5eac41, 0fd2cf2): work content rides footer-less `chore:` daemon commits; the footer lands on a docs/marker commit. Forensics work only because messages cite the daemon SHAs — prose, not machine-readable. Protocol question re-asked five times, never answered (§g2).
