...
> **ARCHIVED 2026-09-21 (docs-health archive sweep)** — every forward-looking item verified resolved at HEAD (per-item triage): all §f items shipped or routed per the 14 Sep in-file strikes (rows 21/25/46/47/83/84/92); TQ_DB guardrail + --wait shipped. No open residue.
...
_Prepared per the done-prompt contract: report file exists at `docs/status/archived/2026-09-10_05-29_readme-overhaul-tqdb-incident.md`; next items above are harvestable (unblocked ones carry no `— BLOCKED:` marker only where they are genuinely agent-executable). Waiting for instructions._
