...
_(Archived 2026-09-14 docs-health pass: window complete; every item resolved-or-routed, verified against HEAD. a) shipped (a8ba7c3/0e8ecfb). b) all rows resolved: ci-local is the standard pre-push gate; TQ_RESULT adoption proven by years of live pool runs (the result-line contract now gates reviews/status/dlqfix/prioritize — AGENTS payload contracts); BLOCKED pinning done (harvest tests); old reports annotated by the subsequent docs-health passes. c) item 1 shipped (TQ_RESULT documented in AGENTS payload contracts); 2 shipped (contract tests incl. placeholder/parse pins); 3-4 superseded/ROADMAP (prompt version markers NOT adopted; forensics ride the review session contract); 5 owned by the lint-baseline policy. §f brainstorm: items 1-8 executed in later windows (4/5/6/7 verified shipped; 8 = the 2026-09-07 harvest runs); 9-50 are verification/ROADMAP fuel routed by the harvest — notable shipped: 37 (`.tq-verify` contract taught in prompts, AGENTS), 15 superseded by the session-close bridge (`--session` resume replaces version markers), 21 superseded (DOMAIN_LANGUAGE carries the session-bridge review/status terms; a dedicated "self-report" glossary entry was not adopted), 26 superseded (the strict TQ_RESULT line contract is pinned by tests and prompt text). g) g1 moot (cqa bridge ships with stub-mode verification; no live CQA server was ever provided); g2 moot (pool runs continuously under systemd, restarts pick up prompts automatically); g3 ANSWERED by practice — authored commits per task with the Task-Queue-ID footer are now the convention (commit-msg hook enforces well-formed footers). Point-in-time snapshot.)_
