...
_(Archived 2026-09-14 docs-health pass: window complete; every item resolved-or-routed, verified against HEAD. a) all shipped — the BLOCKED-filter code fix was ratified by use (the marker is enforced by the harvester and documented in AGENTS); the 127 annotations stand. b) rows: annotation-depth residue owned by the cluster-note archive convention (sanctioned by the 2026-09-13 08-49 pass and the owner-bar question §g1 of that report); the five hearsay-routed items were re-verified in later windows (sidecar retention shipped, ExtractResultPayload fuzz seeds committed, `tq audit --json` shipped, webui free-port shipped, `--verbose` request logging shipped — see the 2026-09-09 §f annotations in the 21-19/23-08 reports); dprint decided MANUAL (AGENTS 2026-09-08 ruling); push state moot (everything pushed long since). c) everything routed and since executed: ci-local.sh (115854a), ctx guard (aaabe1c), nix-binary-runs, v0.2.0 cut 2026-09-09 (later v0.3.0), lint endgame decided (b), nightly fuzz shipped, Windows honesty shipped, the ROADMAP arcs (26-31) remain ROADMAP raw ideas by design. f) [TODO_LIST] items 1-22 all executed in later windows (verified via the TODO_LIST [x] history and the 2026-09-09 §f annotations); [ROADMAP] items 23-50 live in ROADMAP packs (routing verified). g) g1 resolved by operating practice (pool has managed this repo since 2026-09-07, budget-capped, `.crushrc` managed block); g2 resolved (v0.2.0 cut 2026-09-09, full go-release lifecycle, then v0.3.0 2026-09-13); g3 ANSWERED — option (b), the checked-in baseline gate. Point-in-time snapshot.)_
