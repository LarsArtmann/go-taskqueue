...
_(Archived 2026-09-14 docs-health pass: window complete; every item resolved-or-routed, verified against HEAD. a) all shipped (both agent completions cited inline). b) campaign finished long since (the pool has dogfooded this repo daily since); ci-local end-to-end runs are the standard pre-push gate (later hardened: identity-blind release-gates smoke, CI-state check); LAN exposure resolved by the token-auth security model (SECURITY.md, g2 below); post-run review institutionalized as the review/dlqfix/status sweeper pipeline (AGENTS payload contracts); push state moot. c) all shipped-or-resolved: v0.2.0 cut 2026-09-09 (v0.3.0 2026-09-13), CQA replaced by the papdashboard/cqa bridges' stub-verified state, systemd handover shipped (`tq bootstrap --install` + NixOS module, g1 below), DLQ/rescue exercised (`--dlq-fix` autopsy sweeper), budget telemetry superseded by budget-gated minting + papdashboard alerts. §f: the 2026-09-09 annotation above verified the shipped set against HEAD; items 2/3/5/7/8/9 were executed or superseded in later windows (ci-local runs standard, staged-tree concern mooted by the pre-commit hook + daemon ruling, second-device proof superseded by default-deny token auth, C22 review = the review sweeper, budget = mintPass gating, DLQ drill = dlqfix, top running-item = `tq top` live view, crushrc guardrail = prompt contract shipped in the 21-44 session); the [POOL] tail (10-31) all executed (verified via TODO_LIST [x] history); [ROADMAP] rows (36-50) live in ROADMAP packs. g1 shipped-and-answered (bootstrap --install + NixOS module; evo-x2 cutover remains owner-run; budget = `--daily-budget` + mintPass); g2 RESOLVED inline; g3 resolved by the repo-managed `.crushrc` block decision (model+effort live there only, AGENTS). Point-in-time snapshot.)_
...
| Work                                                                                                                                                                                                                                                                                          | Evidence                                                                                                                      |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Round-4 plan written: Pareto tiers, 24 coarse tasks (ALL 22 TODO items mapped to an owner), ~100 fine tasks, mermaid execution graph, launch command, guardrails                                                                                                                              | `docs/planning/archived/2026-09-07_20-47_SUPERB-PLAN-ROUND4-DOGFOOD-POOL-EATS-THIS-REPO.md`                                   |
