...
_(Archived 2026-09-14 docs-health pass: window complete; every item resolved-or-routed, verified against HEAD. a) shipped — StatCard was later retired for the nowband and the adoption table + guard tests are the living record (AGENTS templ-components section). b) all resolved: LAN restart superseded by the token-authed serve model (SECURITY.md); new mapping helpers are table-tested and the adoption table is guard-test-pinned (`TestAdoptionTableCoversTemplates`); design-QA breadth superseded by the a11y/UX bar of later rounds and the CSS drift gate; FEATURES web-UI rows shipped; pool state moot. c) all shipped-or-resolved: W16 token auth (default-deny non-loopback), CSP (nonce-CSP in securityHeaders, smoke-asserted), v0.2.0 cut 2026-09-09, push state moot, ci-local standard, owner questions answered below, journal click-through shipped (fact-feed → task links). §f: the 2026-09-09 annotation above verified 17 items shipped against HEAD; #8 v0.2.0 SHIPPED 2026-09-09 (later v0.3.0); #4/#5/#6 executed in later windows; #11-#50 tail lives in ROADMAP's web-UI polish pack (routing verified by the docs-health audits) with notable ships: #29 tailwind fallback, #30 CONTRIBUTING CSS step, #34 `--open` NOT adopted, #36 demo-video flow = the website-launch skill, #47 check-webui-css gate, #48 retry visibility shipped (webui detail retry strip, `retryTrail` incl. dead-lettered reasons). g1 moot (serve runs supervised under systemd, redesigned binary long since); g2 ANSWERED (token auth + SECURITY.md matrix); g3 moot (v0.2.0/v0.3.0 cut). Point-in-time snapshot.)_
