...
_(Archived 2026-09-14 docs-health pass: window complete; every §f item resolved-or-routed, verified against HEAD. #1 `scripts/ci-local.sh` shipped and is the documented pre-push gate; #2 ctx guard fixed by the pool (`aaabe1c`, 21-19 report); #3 lint endgame DECIDED — option (b): advisory + checked-in shrinking baseline gate (`.golangci-baseline.txt`, `lint-baseline.sh --check`, AGENTS round-13 T5); #4 done (struck in the 18-41 report); #6 superseded by the baseline-file policy (findings are tracked in-repo, CI annotations no longer the surface); #7 push workflow RESOLVED — direct-push + push-then-verify remains the accepted operating model (AGENTS daemon/dogfood sections); #9/#10 shipped (`082e54b` cmd refactor, `5be4c70` CLI tests — 23-08 report b5); #11 resolved by documentation (AGENTS known issue: templ LSP false positives, trust the CLI); #12 decided MANUAL (AGENTS docs-formatting ruling 2026-09-08); #13 NOT-DO — no concurrency group adopted; #14 superseded by the advisory + baseline-gate policy; #25/#26/#28 shipped (govulncheck + gosec jobs in ci.yml, `.github/dependabot.yml`, `tq version` via cmdVersion); #29 shipped (scripts/fuzz/nightly.sh + committed corpus); #30 shipped (`//go:build unix` honesty + windows CI jobs); #31 superseded — the lint-policy ruling is recorded in AGENTS.md, not an ADR; #34 NOT-DO — not adopted; #38 shipped (docs/status/README.md newest-first index); #18 sentinel errors shipped (`internal/task/errors.go` convention); #32/#33/#40 struck above; #17 shipped (gosec triage encoded as excludes, AGENTS 2026-09-10 ruling O5); #16 shipped (`TQ_BIN` smokes the nix-built binary, scripts/smoke/webui.sh); #15 routed (`--all-systems` stays ROADMAP); remaining rows (19-24, 27, 35-37, 39) routed by the 2026-09-07 owner-directed harvest into TODO_LIST (subsequently executed: cyclop cmdHarvest/aggregateTop, pap-e2e, audit failure reporting) and ROADMAP packs. b) all rows resolved by the items above (nix-binary smoke = `checks.binary-runs` + `TQ_BIN` smokes; CHANGELOG versioning = v0.1.0-v0.3.0 all cut). g2 ANSWERED: option (b) — see #3; g3 ANSWERED: direct-push accepted. Point-in-time snapshot.)_
